...
class unetConv2(nn.Module):
    def __init__(self, in_size, out_size, kernel_size, is_batchnorm):
        super(unetConv2, self).__init__()
        # self.channelattention=ChannelAttention(out_size, outsize, ratio=16)
        if is_batchnorm:
            ...
            self.conv2 = nn.Sequential(
                nn.Conv2d(out_size, out_size, kernel_size, 1, 1),
                nn.InstanceNorm2d(out_size),
                nn.ReLU(),
            )
        else:
            self.conv1 = nn.Sequential(nn.Conv2d(in_size, out_size, kernel_size, 1, 1), nn.ReLU())
            self.conv2 = nn.Sequential(
                nn.Conv2d(out_size, out_size, kernel_size, 1, 1), nn.ReLU()
            )
    ...
    def forward(self, inputs):
        ...
        # outputs = self.conv2(outputs)+self.channelattention(outputs)
        #print (outputs.shape)
...
class unetUp(nn.Module):
    def __init__(self, in_size, out_size, is_deconv, is_batchnorm):
        ...
        #     nn.BatchNorm2d(out_size),
        #     nn.ReLU())
        self.conv = unetConv2(in_size, out_size, 3, is_batchnorm)
        if is_deconv:
