...
class Data_Loader():
    ...
    def transform_img(self, resize, totensor, normalize, centercrop,randomrotation, colorjitter,randomaffine):

        options = []

        if randomrotation:
            options.append(transforms.RandomRotation(degrees=30))
        if colorjitter:
            options.append(transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.1))
        if randomaffine:
            options.append(transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.8, 1.2)))
        if centercrop:
    ...
    def transform_label(self, resize, totensor, normalize, centercrop,randomrotation,colorjitter,randomaffine):
    ...
    def loader(self):
        transform_img = self.transform_img(True, True, True, False,True, True,True)
        transform_label = self.transform_label(True, True, False, False,True,False,True)
